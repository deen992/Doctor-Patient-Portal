...
public class UserViewDAO {
	...
	public UserViewDAO(Connection conn) {
		super();
		this.conn = conn;
	}
	...
	public List<User> getAllUser() {
	    ...
	    User user = null;
	    try {
	        String sql = "select * from user_details order by id desc"; // Order by ID descending for newest first
	        PreparedStatement pstmt = conn.prepareStatement(sql);
	        ...
	        while (rs.next()) {
	            ...
	            user.setPassword(rs.getString(4)); // Be careful displaying passwords in a UI!
	            list.add(user);
	        }
	    } catch (Exception e) {
	        e.printStackTrace();
